...
#   ./examples/run.sh convert    # run a single example (convert|edit|merge-split|transfer)
#
...
case "${1:-all}" in
  all)
    ...
    run transfer
    ;;
  convert | edit | merge-split | transfer)
    run "$1"
    ;;
  *)
    echo "unknown example: $1" >&2
    echo "usage: $0 [all|convert|edit|merge-split|transfer]" >&2
    exit 1
    ;;
esac

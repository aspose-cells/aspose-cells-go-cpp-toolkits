...
// Config holds the native-typed option values for Xls save options.
//
...
func (c *Config) Apply(source []byte) ([]byte, error) {
	opts, err := asposecells.NewXlsSaveOptions()
	if err != nil {
		return nil, err
	}

	if c.matchColor != nil {
		if err := opts.SetMatchColor(*c.matchColor); err != nil {
			return nil, err
		}
	}
...
func init() {
	formats.Register("xls", func() saveoptions.SaveOption {
		return New()
	})
}

// New creates a new instance of xls save options
//
// The New function creates an instance of xls SaveOption using the Functional Options Pattern. This function accepts a variable number of Option function parameters, and each Option function modifies the configuration of SaveOption.
//
// Parameters:
//
...
//
// Return value:
// xls SaveOption - Configured instance of the saved option
//
// Usage example:
//

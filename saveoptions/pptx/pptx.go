...
// Config holds the native-typed option values for Pptx save options.
//
...
type Config struct {
	ignoreHiddenRows                  *bool
	saveAsEditableShapes              *bool
	embedXlsxAsChartDataSource        *bool
	adjustFontSizeForRowType          *asposecells.AdjustFontSizeForRowType
	exportViewType                    *asposecells.SlideViewType
	asFlatOpc                         *bool
	defaultFont                       *string
...
func (c *Config) Apply(source []byte) ([]byte, error) {
	opts, err := asposecells.NewPptxSaveOptions()
	if err != nil {
		return nil, err
	}

	if c.ignoreHiddenRows != nil {
		if err := opts.SetIgnoreHiddenRows(*c.ignoreHiddenRows); err != nil {
			return nil, err
		}
	}
	...
	if c.adjustFontSizeForRowType != nil {
		if err := opts.SetAdjustFontSizeForRowType(*c.adjustFontSizeForRowType); err != nil {
			return nil, err
		}
	}
	if c.exportViewType != nil {
		if err := opts.SetExportViewType(*c.exportViewType); err != nil {
			return nil, err
		}
	}
	if c.asFlatOpc != nil {
...
// New creates a new instance of pptx save options
//
// The New function creates an instance of pptx SaveOption using the Functional Options Pattern. This function accepts a variable number of Option function parameters, and each Option function modifies the configuration of SaveOption.
//
// Parameters:
//
...
//
// Return value:
// pptx SaveOption - Configured instance of the saved option
//
// Usage example:
//

...
// Config holds the native-typed option values for Dif save options.
//
...
// Apply processes the given source byte slice as a Dif file and returns the converted output.
// This method satisfies the saveoptions.SaveOption (or equivalent) interface, enabling Dif-specific export logic.
//
// Parameters:
// - source: A byte slice representing the input spreadsheet or data source. The implementation may interpret
// this as an intermediate format (e.g., XLSX or CSV bytes) and convert it into Dif format.
//
// Returns:
// - []byte: The resulting Dif file content as a byte slice.
// - error: error information.
func (c *Config) Apply(source []byte) ([]byte, error) {
	opts, err := asposecells.NewDifSaveOptions()
	if err != nil {
		return nil, err
	}
